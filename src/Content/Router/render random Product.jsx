...
function ProductRandom({ children }) {
    ...
    return <>
        {shuffle(children.children).map((items) => {
            return (
                <>
                    {children.indexPage == items.id ? <div   style={{ display: "none" }}>

                    </div> :
                        <div className={clsx(cssItems.container_Items)} >
                            <Link to={`/song ${items.title}`} onClick={() => { setTimeout(() => { location.reload() },0) }}
